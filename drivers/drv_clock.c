...
{
    rt_uint32_t cpu_src;
    int best_k, best_m, best_n, pre_clk, max = 0;
    ...
    CCU->pll_cpu_ctrl = (0x1 << 31) | (best_m << 0) | (best_k << 4) | (best_n << 8);
...
{
	int best_m, best_n, pre_clk, max = 0;
    ...
    CCU->pll_audio_ctrl &= ~(0x1 << 31);
    CCU->pll_audio_ctrl = (0x1 << 31) | (0x0 << 24) | (best_n << 8) | best_m;
