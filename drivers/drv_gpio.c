...
rt_err_t gpio_set_value(enum gpio_port port, enum gpio_pin pin, rt_uint8_t value)
{
    rt_uint32_t addr;
    rt_uint32_t offset;
    rt_uint32_t data;
...
rt_err_t gpio_get_value(enum gpio_port port, enum gpio_pin pin)
{
    rt_uint32_t addr;
    rt_uint32_t offset;
    rt_uint32_t data;
...
rt_err_t gpio_set_pull_mode(enum gpio_port port,  enum gpio_pin pin, enum gpio_pull pull)
{
    rt_uint32_t addr;
    rt_uint32_t offset;
    rt_uint32_t data;
...
rt_err_t gpio_set_drive_level(enum gpio_port port, enum gpio_pin pin, enum gpio_drv_level level)
{
...
void gpio_irq_enable(enum gpio_port port,  enum gpio_pin pin)
{
	...
	gpio_set_func(port, pin, IO_FUN_5);
    LOG_D("[line]:%d offset:%d addr:%08x data:%08x", __LINE__, offset, addr, *((rt_uint32_t *)addr));
}
...
void gpio_irq_disable(enum gpio_port port,  enum gpio_pin pin)
{
    ...
    writel(data, addr);
	gpio_set_func(port, pin, IO_DISABLE);
    LOG_D("[line]:%d offset:%d addr:%08x data:%08x", __LINE__, offset, addr, *((rt_uint32_t *)addr));
}
...
static void pin_mode(struct rt_device *dev, rt_base_t pin, rt_base_t mode)
{
    ...
    {
        mode = IO_INPUT;
        gpio_set_pull_mode(pin_index[pin].pin_port, pin_index[pin].pin, PULL_DISABLE);
    }
    else if(mode == PIN_MODE_INPUT_PULLUP)
    {
        mode = IO_INPUT;
        gpio_set_pull_mode(pin_index[pin].pin_port, pin_index[pin].pin, PULL_UP);
    }
    else if(mode == PIN_MODE_INPUT_PULLDOWN)
    {
        mode = IO_INPUT;
        gpio_set_pull_mode(pin_index[pin].pin_port, pin_index[pin].pin, PULL_DOWN);
    }
